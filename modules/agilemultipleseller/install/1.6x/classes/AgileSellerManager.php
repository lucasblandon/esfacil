...
class AgileSellerManagerCore
{
	...
	private static function assignObjectOwnerShared($table,$xr_table, $id, $id_seller)
	{
		...
		{
			$sql = 'UPDATE '._DB_PREFIX_.'object_owner SET id_owner=' . $id_seller . ' WHERE  `entity`=\'' . $table . '\' AND id_object=' . intval($id);
		}
		else
		{
	...
	public static function getProducts($id_seller, Context $context, $nb_only=false, $p=1, $n=10, $orderby='', $orderway='')
	{
		$select = '
		...
		$res = Product::getProductsProperties(Context::getContext()->cookie->id_lang, $res);
