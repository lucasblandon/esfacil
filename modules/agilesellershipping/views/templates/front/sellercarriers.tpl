...
<div class="block-center clearfix" id="block-history">
    <div class="row">
		<a class="agile-btn agile-btn-default" href="{$link->getModuleLink('agilesellershipping', 'sellercarrierdetail', ['id_carrier' =>0], true)}" class="button">
    ...
    <table id="carrier-list" class="std">
        <thead>
	        <tr>
    	...
    	{assign var='detail_url' value=$link->getModuleLink('agilesellershipping', 'sellercarrierdetail', ['id_carrier' => $carrier.id_carrier], true)}
	        <tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
		        ...
		        <td><a href="{$detail_url}">{$carrier.name}</a></td>
